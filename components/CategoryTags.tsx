...
const CategoryTags: React.FC = () => {
  ...
  useEffect(() => {
    // Scroll to the active tag horizontally after categories are fetched
    if (activeTagRef.current && categoryContainerRef.current) {
      categoryContainerRef.current.scrollTo({
        left: activeTagRef.current.offsetLeft - categoryContainerRef.current.offsetLeft,
        behavior: 'auto',
      });
    }
  }, [categories, router.asPath]); // Make sure to run when the category list or active route changes

  return (
    <div className="category-tags" ref={categoryContainerRef}>
      {isLoading ? (
        <div className="category-tags-loading">
          {/* Skeleton loading states */}
          <div className="loading-tag" style={{ width: '50px', height: '35px' }}></div>
        ...
        <>
          ...
          {categories.map((category) => {
            ...
            return (
              <Link key={category.slug} href={`/${category.slug}`}>
                <div
                  ref={isActive ? activeTagRef : null} // Only set the ref for the active tag
                  className={`category-tag ${isActive ? 'active' : ''}`}
                >
                  {category.name}
                </div>
